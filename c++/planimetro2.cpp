...
double calculaPonto2(vector<point> curva, int idx, int s);
double calculaPonto3(vector<point> curva, int idx, int s);
...
void printResultado() {
    printf("Green1: %f, Erro: %f\n",resGreen, (A - resGreen));
    printf("Green2: %f, Erro: %f\n",resGreen2, A - resGreen2);
    printf("Green3: %f, Erro: %f\n",resGreen3, A - resGreen3);
    printf("Green4: %f, Erro: %f\n",resGreen4, A - resGreen4);
...
double calculaPonto(vector<point> curva, int idx, int s) {
	...
	double cosTheta = -(c*u+d*v)/(sqrt(c*c + d*d)*sqrt(u*u + v*v));
	return cosTheta*dist(p1.x,p1.y,p2.x,p2.y)*(r/2);
}
...
double calculaPonto3(vector<point> curva, int idx, int s) {
	...
	double cosTheta = -(c*u+d*v)/(sqrt(c*c + d*d)*sqrt(u*u + v*v));
	return cosTheta*dist(p1.x,p1.y,p0.x,p0.y)*(r/2);
}
...
void cGreen(vector<point> curva) {
	...
	for (int i = 0; i < s; i++) {
		...
		c = -(y-b)/(r/2);
        d = (x-a)/(r/2);
		res1 += calculaPonto(curva, i, s);
        res2 += calculaPonto2(curva, i, s);
        res3 += calculaPonto3(curva, i, s);
	}

	resGreen = res1;
    resGreen2 = res2;
    resGreen3 = (res1 + res2)/2;
    resGreen4 = res3;
}
